...
<li class="w-full">
  <button
    ...
    <div
      class="mr-0 flex h-10 w-10 items-center justify-center rounded-md bg-[#5ca828] text-white md:mr-4">
      {index + 1}
    </div>
    ...
    <svg
      ...
      <polygon points="0,6 12,18 24,6" fill="#5ca828" />
    </svg>
  </button>
  {#if !isCollapsed}
    ...
    <ul>
      {#each retrievedSnippets as snippet, i (snippet.uri)}
        <li
          class="relative flex overflow-hidden border-b p-4 first:rounded-lg first:border first:border-[#5ca828] last:border-b-0"
